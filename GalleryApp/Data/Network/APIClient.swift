...
class APIClient {
    ...
    init() {
        guard let path = Bundle.main.path(forResource: "Config", ofType: "plist"),
            ...
            fatalError("Config.plist не найден или не найден UnsplashAccessKey")
        }
        self.apiKey = key
    }
    ...
    func fetchPhotos(page: Int, perPage: Int) async throws -> [PhotoResponse] {
        ...
        guard let url = URL(string: endpoint) else {
            throw NSError(domain: "NetworkError", code: 400, userInfo: [NSLocalizedDescriptionKey: "Некорректный URL"])
        }
        ...
        guard let response = response as? HTTPURLResponse, response.statusCode == 200 else {
            throw NSError(domain: "NetworkError", code: 400, userInfo: [NSLocalizedDescriptionKey: "Некорректный response"])
        }
